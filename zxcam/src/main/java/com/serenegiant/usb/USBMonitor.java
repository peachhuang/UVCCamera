...
import com.serenegiant.common.UvcDebug;
...
public final class USBMonitor {
	...
	public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
		if (UvcDebug.DEBUG) Log.v(TAG, "USBMonitor:Constructor");
		if (listener == null)
		...
		destroyed = false;
		if (UvcDebug.DEBUG) Log.v(TAG, "USBMonitor:mUsbManager=" + mUsbManager);
	}
	 ...
	 */
	public void destroy() {
		if (UvcDebug.DEBUG) Log.i(TAG, "destroy:");
		unregister();
		if (!destroyed) {
			destroyed = true;
	...
	public synchronized void register() throws IllegalStateException {
		...
		if (mPermissionIntent == null) {
			if (UvcDebug.DEBUG) Log.i(TAG, "register:");
	...
	public synchronized void unregister() throws IllegalStateException {
		...
		if (mPermissionIntent != null) {
//			if (UvcDebug.DEBUG) Log.i(TAG, "unregister:");
			final Context context = mWeakContext.get();
			try {
				if (context != null) {
...
//		if (UvcDebug.DEBUG) Log.v(TAG, "requestPermission:device=" + device);
	...
	private final void processConnect(final UsbDevice device) {
		...
		mAsyncHandler.post(new Runnable() {
			@Override
			public void run() {
				if (UvcDebug.DEBUG) Log.v(TAG, "processConnect:device=" + device);
				UsbControlBlock ctrlBlock;
	...
	private final void processCancel(final UsbDevice device) {
		if (destroyed) return;
		if (UvcDebug.DEBUG) Log.v(TAG, "processCancel:");
	...
	private final void processAttach(final UsbDevice device) {
		if (destroyed) return;
		if (UvcDebug.DEBUG) Log.v(TAG, "processAttach:");
		if (mOnDeviceConnectListener != null) {
			mAsyncHandler.post(new Runnable() {
				@Override
				public void run() {
	...
	private final void processDettach(final UsbDevice device) {
		if (destroyed) return;
		if (UvcDebug.DEBUG) Log.v(TAG, "processDettach:");
		if (mOnDeviceConnectListener != null) {
			mAsyncHandler.post(new Runnable() {
				@Override
				public void run() {
...
//		if (UvcDebug.DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
		return sb.toString();
	}
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			if (UvcDebug.DEBUG) Log.i(TAG, "UsbControlBlock:constructor");
...
//			}
		...
		public synchronized void close() {
			if (UvcDebug.DEBUG) Log.i(TAG, "UsbControlBlock#close:");
