...
static int op_init2(struct libusb_context *ctx, const char *usbfs) {	// XXX
	...
	r = LIBUSB_SUCCESS;
	if (init_count == 0) {
		usbi_dbg("start up hotplug event handler");
	...
	if (r == LIBUSB_SUCCESS) {
		usbi_dbg("call android_scan_devices");
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer) {
	...
	usbi_dbg("need %d urbs for new transfer with length %d , bulk_buffer_len = %d, dpriv->caps = 0x%x", num_urbs, transfer->length, 
		bulk_buffer_len, dpriv->caps);
...
#if SONIX_MT3561
		//op_clear_halt(handle, urb->endpoint);	// XXX added saki	//cys by sonix
#else
		op_clear_halt(handle, urb->endpoint); // XXX added saki	//cys by sonix
#endif
		goto cancel_remaining;
