...
#include <libgen.h>
	...
	#define UVC_ENTER() LOGD("begin %s", __FUNCTION__)
	#define UVC_EXIT(code) LOGD(" end %s (%d)", __FUNCTION__, code)
	#define UVC_EXIT_VOID() LOGD(" end %s", __FUNCTION__)
#else
	#define UVC_DEBUG(format, ...) fprintf(stderr, " " format "\n", ##__VA_ARGS__)
	#define UVC_ENTER() fprintf(stderr, "begin %s\n", __FUNCTION__)
	#define UVC_EXIT(code) fprintf(stderr, " end %s (%d)\n", __FUNCTION__, code)
	#define UVC_EXIT_VOID() fprintf(stderr, " end %s\n", __FUNCTION__)
#endif
#else
	#define UVC_DEBUG(...)
	#define UVC_ENTER()
