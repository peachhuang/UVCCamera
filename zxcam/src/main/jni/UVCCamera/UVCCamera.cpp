...
int UVCCamera::startPreview() {
    ...
    x = 1 + rand()%126;
    //__android_log_print(ANDROID_LOG_INFO, "-", "H: 0x%x, P 0x%x", PU_HUE,mPUSupports);
#if ENCRYPT	//encry
	if (mDeviceHandle && (mPUSupports & PU_HUE)) {
		xwHue = getHue();
		//t2 = xwHue;//del-me
		setGamma(x);
		usleep(1*1000);
		...
		xwHue = xwHue%126 + 1;
		//
		//LOGE("check err! %d , %d, %d",vHue,xwHue,x);
		__android_log_print(ANDROID_LOG_INFO, "-", " %d , %d, %d",vHue,xwHue,x);
		//__android_log_print(ANDROID_LOG_INFO, "-", "init-hue=%d, remote-vHue %d, ,local-Hue %d, x-gamma = %d",t2,vHue,xwHue,x);
		if(xwHue != vHue){
			return -1;
		}
...
#else
	if (mDeviceHandle) {
		return mPreview->startPreview();//(this);
	}
#endif
...
int UVCCamera::getBrightness() {
    ...
    x = 1 + rand()%126;
		xwHue = getHue();
		    LOGE("getBrightness, xwHue %d", xwHue);
#if 1
		setGamma(x);
		usleep(1*1000);
		vHue = getHue();
		...
		xwHue = xwHue%126 + 1;
		//
		LOGE("getBrightness check err! %d , %d, %d",vHue,xwHue,x);
		if(xwHue != vHue){
			return -1;
		}
#endif
