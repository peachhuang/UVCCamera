...
#if UVCPREVIEW_LOCAL_DEBUG	// set 1 if you don't need debug log
	#ifndef LOG_NDEBUG
...
#include "../ffmpeg/ffmpeg_opt.h"

#define MAX_FRAME 4
#define PREVIEW_PIXEL_BYTES 4	// RGBA/RGBX
#define FRAME_POOL_SZ MAX_FRAME + 2
unsigned int alloc_frame = 0;
...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode, float bandwidth) {
	...
	if ((requestWidth != width) || (requestHeight != height) || (requestMode != mode)) {
		...
		LOGI("setsize start, requestWidth=%d, requestHeight=%d",requestWidth, requestHeight);
		//mDeviceHandle->info->stream_ifs->format_descs->frame_descs->
...
int UVCPreview::startPreview(){//(UVCCamera ucam) {
	ENTER();
...
int UVCPreview::stopPreview() {
	...
	if (LIKELY(b)) {
		mIsRunning = false;
		alloc_frame = 0;
#if UVCPREVIEW_LOCAL_DEBUG
		LOGE("#### stopPreview mIsRunning = false, alloc_frame=0");
#endif
		pthread_cond_signal(&preview_sync);
		pthread_cond_signal(&capture_sync);
...
//char *PATH = "/sdcard/DCIM/ffpeg100/ffp";
//char pathf[50] = "/sdcard/DCIM/ffpeg100/ffp";
char *PATH = "/sdcard/z";//for redmi-3
char pathf[50] = "/sdcard/z";
//char *PATH = "/data/data/tst/ffp";
//char pathf[50] = "/data/data/tst/ffp";
const char *hex = "_1080.jpeg";
unsigned long testL = 0;
char *tmp;
int port;
char *tmp1;
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	...
	testL = 0;
	LOGI("do_preview... result=%d", result);
	//convert_init();
//	ffmpeg_test();
	...
	if (LIKELY(!result)) {
		...
		LOGI("Streaming...");
#endif
		LOGE(" frameMode %d", frameMode);
		if (frameMode) {
			// MJPEG mode
			for ( ; LIKELY(isRunning()) ; ) {
				//MARK("mark wait start");
			    //LOGE("wait start...");
				frame_mjpeg = waitPreviewFrame();
				if (LIKELY(frame_mjpeg)) {
				//LOGE("frame_mjpeg start... data=%d, actual= %d", frame_mjpeg->data_bytes, frame_mjpeg->actual_bytes);

				//if(recording == true && testL < 1800){
				//if(testL < 500 && testL > 400){
				testL++;
				if(testL == 100){
					testL++;
					...
					strcat(pathf,tmp);
					strcat(pathf,hex);
					FILE *fp = fopen(pathf, "w+");
					//FILE *fp = fopen("/data/data/m.jpeg", "w+");
					if (fp != NULL) {
						LOGE("data_bytes[%d],%d x %d",frame_mjpeg->data_bytes,frame_mjpeg->width,frame_mjpeg->height);	
						fwrite(frame_mjpeg->data, frame_mjpeg->data_bytes, 1, fp);	
						fclose(fp); 
					} else
						LOGE("create ffpeg fail--%s",pathf);
				}
					
				#if 0//ZXCZ_MJ_YUV_RGB
					//LOGE("#### frame_format:%d",frame_mjpeg->frame_format);
					frame = get_frame(frame_mjpeg->width * frame_mjpeg->height * 2);
					//LOGI("uvc_mjpeg2yuyv start...");
					result = uvc_mjpeg2yuyv(frame_mjpeg, frame);   // MJPEG => yuyv
					//LOGI("uvc_mjpeg2yuyv end...");
					recycle_frame(frame_mjpeg);
					if (LIKELY(!result)) {
						frame = draw_preview_one(frame, &mPreviewWindow, uvc_any2rgbx, 4);
						alloc_frame++;	//cys
						//LOGE("#### do_preview, mIsRunning=%d alloc_frame=%d", mIsRunning, alloc_frame);
						addCaptureFrame(frame);
					} else {
						//LOGE("#### result fail. MJ to YUV error");
						recycle_frame(frame);
					}
					//alloc_frame++;	//cys
					#if UVCPREVIEW_LOCAL_DEBUG
					...
					#endif
					//LOGE("#### , frame_format=%d", frame_mjpeg->frame_format);
					frame_mjpeg = draw_preview_one(frame_mjpeg, &mPreviewWindow, uvc_any2rgbx, 4);//2*4);
					addCaptureFrame(frame_mjpeg);
				#endif
			//	LOGI("frame_mjpeg end...");
				}
				else
				{
					#if UVCPREVIEW_LOCAL_DEBUG
					LOGE("#### do_preview, error mIsRunning=%d alloc_frame=%d", mIsRunning, alloc_frame);
					#endif
				}
			}
		} else {
...
int copyToSurface(uvc_frame_t *frame, ANativeWindow **window) {
	...
	if (LIKELY(*window)) {
		...
		if (LIKELY(ANativeWindow_lock(*window, &buffer, NULL) == 0)) {
			//LOGE("#### copyToSurface draw one frame");
			// source = frame data
			//LOGE("copyToSurface 00");
			...
			//LOGE("copyToSurface 01");
			const int w = src_w < dest_w ? src_w : dest_w;
			// use lower height
			const int h = frame->height < buffer.height ? frame->height : buffer.height;
			// transfer from frame data to the Surface
			copyFrame(src, dest, w, h, src_step, dest_step);
			//LOGE("copyToSurface 02");
			ANativeWindow_unlockAndPost(*window);
			//LOGE("copyToSurface 03");
		} else {
			result = -1;
		}
	} else {
...
uvc_frame_t *UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window, convFunc_t convert_func, int pixcelBytes) {
	...
	if (LIKELY(b)) {
		...
		uvc_frame_t *frameYUV;
		uvc_frame_t *frameRGB;
		uvc_frame_t *frameRGB1;
		FILE *fp;
		if (convert_func) {
...
/*				//LOGI("yuv2rgb start...");
				...
				converted->height = frame->height;
					...
					frameRGB1 = get_frame(frame->width * frame->height * 4);
					frameRGB1->width = frame->width;
					frameRGB1->height = frame->height;
					uvc_mjpeg2rgbx(frame, frameRGB1);//frameRGB1);
										fp = fopen("/sdcard/DCIM/ffmpeg/RGBX-dir.jpg", "w+");
										if (fp != NULL) {
											fwrite(frameRGB1->data, frame->width * frame->height * 4, 1, fp);	
											fclose(fp); 
										} else
											LOGE("scale.jpg fail!");

					recycle_frame(frameRGB1);
					frameYUV = get_frame(frame->width * frame->height * 2);
					//LOGI("uvc_mjpeg2yuyv start...");
					uvc_mjpeg2yuyv(frame, frameYUV);   // MJPEG => yuyv
					  					fp = fopen("/sdcard/DCIM/ffmpeg/yuv.jpg", "w+");
										if (fp != NULL) {
											fwrite(frameYUV->data, frameYUV->width * frameYUV->height * 2, 1, fp);	
											fclose(fp); 
										} else
											LOGE("scale.jpg fail!"); 
					recycle_frame(frameYUV);
					frameRGB = get_frame(frame->width * frame->height * 4);
					uvc_yuyv2rgbx(frameYUV, frameRGB);
										fp = fopen("/sdcard/DCIM/ffmpeg/yuvRGBX.jpg", "w+");
										if (fp != NULL) {
											fwrite(frameRGB->data, frameRGB->width * frameRGB->height * 4, 1, fp);	
											fclose(fp); 
										} else
											LOGE("scale.jpg fail!");
					recycle_frame(frameRGB);*/
					//converted = frameRGB1;
					//ffmpeg_yuv2rgb(frameYUV->data,converted->data,frameYUV->data_bytes);
					//processimage(frame->data, converted->data,frame->data_bytes);
				//b = ffmpeg_mjpeg2rgb(frame->data,converted->data,frame->data_bytes);
				b = convert_func(frame, converted);
				//converted->data = rgb888_to_rgbx8888_buffer(converted->data, 1280, 720);
				//LOGI("yuv2rgb end...");
				//b=0;
				if (!b) {
					pthread_mutex_lock(&preview_mutex);
					if(false){//(nframes%200 == 0 && skipf < 30){
					    skipf++;
					}
					else{
					    skipf = 1;
