...
static jint nativeConnect(JNIEnv *env, jobject thiz,
	...
	ENTER();
	int result = JNI_ERR;
	//if(vid != 0xc45 || pid != 0x6366)RETURN(result, jint);
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
...
static jint nativeSetPreviewDisplay(JNIEnv *env, jobject thiz,
	...
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	if (LIKELY(camera)) {
	   // LOGE("jSurface is:%s", (jSurface == NULL? "NULL":"not null"));
		ANativeWindow *preview_window = jSurface ? ANativeWindow_fromSurface(env, jSurface) : NULL;
		//LOGE("after preview_window is:%s", (preview_window == NULL? "NULL":"not null"));
