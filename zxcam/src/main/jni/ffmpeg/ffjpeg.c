...
int ffjpeg_encoder_encode(void *ctxt, const char *file, int w, int h, AVFrame *frame)
{
    ...
    if (  frame->format != encoder->ifmt
       ...
       || w != encoder->ow || h != encoder->oh ) {
        ...
        encoder->sws_ctx = sws_getContext(frame->width, frame->height, (enum AVPixelFormat)frame->format,
        ...
        encoder->ow   = w;
        encoder->oh   = h;
        encoder->ifmt = (enum AVPixelFormat)frame->format;
...
AVFrame *ffjpeg_decoder_decode(void *ctxt, void *buf, int len)
{
    ...
    AVPacket packet;
    int      got = 0;
LOGI("ffjpeg_decoder_decode 01.");
    av_init_packet(&packet);
    LOGI("ffjpeg_decoder_decode 02.");
    packet.data = (uint8_t*)buf;
    packet.size = len;
    LOGI("ffjpeg_decoder_decode 03.");

    avcodec_decode_video2(dec->ctxt, &dec->frame, &got, &packet);
        LOGI("ffjpeg_decoder_decode 04.");
    return &dec->frame;
}
