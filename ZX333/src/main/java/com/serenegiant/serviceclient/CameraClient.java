...
public class CameraClient implements ICameraClient {
	...
	protected void finalize() throws Throwable {
		if (UvcDebug.DEBUG) Log.v(TAG, "finalize");
		doUnBindService();
		super.finalize();
	}
	...
	public void select(final UsbDevice device) {
		if (UvcDebug.DEBUG) Log.v(TAG, "select:device=" + (device != null ? device.getDeviceName() : null));
		mUsbDevice = device;
	...
	@Override
	public void release() {
		if (UvcDebug.DEBUG) Log.v(TAG, "release:" + this);
		mUsbDevice = null;
	...
	public void resize(final int width, final int height) {
		if (UvcDebug.DEBUG) Log.v(TAG, String.format("resize(%d,%d)", width, height));
		final CameraHandler handler = mWeakHandler.get();
	...
	public void removeSurface(final Surface surface) {
		if (UvcDebug.DEBUG) Log.v(TAG, "removeSurface:surface=" + surface + ",hash=" + surface.hashCode());
		final CameraHandler handler = mWeakHandler.get();
	...
	protected void doUnBindService() {
		if (UvcDebug.DEBUG) Log.v(TAG, "doUnBindService:");
		synchronized (mServiceSync) {
			if (mService != null) {
				final Context context = mWeakContext.get();
				if (context != null) {
	...
	private final ServiceConnection mServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(final ComponentName name, final IBinder service) {
			if (UvcDebug.DEBUG) Log.v(TAG, "onServiceConnected:name=" + name);
			synchronized (mServiceSync) {
		...
		public void onServiceDisconnected(final ComponentName name) {
			if (UvcDebug.DEBUG) Log.v(TAG, "onServiceDisconnected:name=" + name);
			synchronized (mServiceSync) {
				mService = null;
				mServiceSync.notifyAll();
			}
		...
		synchronized (mServiceSync) {
			if (mService == null) {
				try {
					...
					if (UvcDebug.DEBUG) Log.e(TAG, "getService:", e);
				}
			}
		}
		return mService;
	...
	private static final class CameraHandler extends Handler {
		...
		public boolean isRecording() {
			...
			try {
				...
				if (UvcDebug.DEBUG) Log.e(TAG, "isRecording:", e);
			}
			return false;
		}
		...
		private static final class CameraTask extends IUVCServiceCallback.Stub implements Runnable {
			...
			@Override
			public void run() {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "run:");
				Looper.prepare();
				synchronized (mSync) {
				...
				Looper.loop();
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "run:finishing");
				synchronized (mSync) {
					mHandler = null;
					mParent = null;
					mSync.notifyAll();
...
// callbacks from service
			...
			public void onConnected() throws RemoteException {
				...
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "onConnected:");
				mIsConnected = true;
				if (mParent != null) {
			...
			public void onDisConnected() throws RemoteException {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "onDisConnected:");
				mIsConnected = false;
				if (mParent != null) {
					if (mParent.mListener != null) {
...
//================================================================================
			public void handleSelect(final UsbDevice device) {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "handleSelect:");
				final IUVCService service = mParent.getService();
				if (service != null) {
					try {
						mServiceId = service.select(device, this);
					} catch (final RemoteException e) {
						if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "select:", e);
					}
				}
			}

			public void handleRelease() {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "handleRelease:");
				mIsConnected = false;
				mParent.doUnBindService();
			}

			public void handleConnect() {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "handleConnect:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					...
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleConnect:", e);
				}
			}

			public void handleDisconnect() {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "handleDisconnect:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					...
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleDisconnect:", e);
				}
				mIsConnected = false;
			}

			public void handleAddSurface(final Surface surface, final boolean isRecordable) {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "handleAddSurface:surface=" + surface + ",hash=" + surface.hashCode());
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.addSurface(mServiceId, surface.hashCode(), surface, isRecordable);
				} catch (final RemoteException e) {
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleAddSurface:", e);
				}
			}

			public void handleRemoveSurface(final Surface surface) {
				if (UvcDebug.DEBUG) Log.v(TAG_CAMERA, "handleRemoveSurface:surface=" + surface + ",hash=" + surface.hashCode());
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.removeSurface(mServiceId, surface.hashCode());
				} catch (final RemoteException e) {
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleRemoveSurface:", e);
				}
			}

			public void handleStartRecording() {
				if (UvcDebug.DEBUG) Log.v(TAG, "handleStartRecording:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					...
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleStartRecording:", e);
				}
			}

			public void handleStopRecording() {
				if (UvcDebug.DEBUG) Log.v(TAG, "handleStopRecording:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					if (service.isRecording(mServiceId)) {
					...
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleStopRecording:", e);
				}
			}

			public void handleCaptureStill(final String path) {
				if (UvcDebug.DEBUG) Log.v(TAG, "handleCaptureStill:" + path);
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.captureStillImage(mServiceId, path);
				} catch (final RemoteException e) {
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleCaptureStill:", e);
				}
			}
			
			public void handleResize(final int width, final int height) {
				if (UvcDebug.DEBUG) Log.v(TAG, String.format("handleResize(%d,%d)", width, height));
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.resize(mServiceId, width, height);
				} catch (final RemoteException e) {
					if (UvcDebug.DEBUG) Log.e(TAG_CAMERA, "handleResize:", e);
				}
			}
		}
	}
