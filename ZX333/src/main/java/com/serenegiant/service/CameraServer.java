...
import com.serenegiant.ZX333.UvcDebug;
...
import com.serenegiant.ZX333.R;
...
public final class CameraServer extends Handler {
	...
	private static final boolean WRITER_FINISHED = true;
	...
	public static CameraServer createServer(final Context context, final UsbControlBlock ctrlBlock, final int vid, final int pid) {
		if (UvcDebug.DEBUG) Log.d(TAG, "createServer:");

//		LogCather.getInstance(context).start();
	...
	private CameraServer(final CameraThread thread) {
		if (UvcDebug.DEBUG) Log.d(TAG, "Constructor:");
	...
	protected void finalize() throws Throwable {
		if (UvcDebug.DEBUG) Log.i(TAG, "finalize:");
		release();
		super.finalize();
	}

	public void registerCallback(final IUVCServiceCallback callback) {
		if (UvcDebug.DEBUG) Log.d(TAG, "registerCallback:");
	...
	public boolean unregisterCallback(final IUVCServiceCallback callback) {
		if (UvcDebug.DEBUG) Log.d(TAG, "unregisterCallback:");
	...
	public void release() {
		if (UvcDebug.DEBUG) Log.d(TAG, "release:");
		disconnect();
		mCallbacks.kill();
...
//********************************************************************************
	public void resize(final int width, final int height) {
		if (UvcDebug.DEBUG) Log.d(TAG, String.format("resize(%d,%d)", width, height));
		if (!isRecording()) {
			mFrameWidth = width;
			mFrameHeight = height;
			if (mRendererHolder != null) {
	...
	public void connect() {
		if (UvcDebug.DEBUG) Log.d(TAG, "connect:");
		...
		if (!thread.isCameraOpened()) {
			...
			if (UvcDebug.DEBUG) Log.d(TAG, "already connected, just call callback");
			processOnCameraStart();
		}
	}

	public void connectSlave() {
		if (UvcDebug.DEBUG) Log.d(TAG, "connectSlave:");
		final CameraThread thread = mWeakThread.get();
		if (thread.isCameraOpened()) {
			processOnCameraStart();
		}
	...
	public void disconnect() {
		if (UvcDebug.DEBUG) Log.d(TAG, "disconnect:");
		stopRecording();
		final CameraThread thread = mWeakThread.get();
	...
	public void addSurface(final int id, final Surface surface, final boolean isRecordable, final IUVCServiceOnFrameAvailable onFrameAvailableListener) {
		if (UvcDebug.DEBUG) Log.d(TAG, "addSurface:id=" + id +",surface=" + surface);
		if (mRendererHolder != null)
			mRendererHolder.addSurface(id, surface, isRecordable);
	}

	public void removeSurface(final int id) {
		if (UvcDebug.DEBUG) Log.d(TAG, "removeSurface:id=" + id);
		if (mRendererHolder != null)
...
//********************************************************************************
	private void processOnCameraStart() {
		if (UvcDebug.DEBUG) Log.d(TAG, "processOnCameraStart:");
		try {
	...
	private void processOnCameraStop() {
		if (UvcDebug.DEBUG) Log.d(TAG, "processOnCameraStop:");
		final int n = mCallbacks.beginBroadcast();
		for (int i = 0; i < n; i++) {
			if (((CallbackCookie)mCallbacks.getBroadcastCookie(i)).isConnected)
			try {
...
//**********************************************************************
	...
	private static final class CameraThread extends Thread {
		...
		private CameraThread(final Context context, final UsbControlBlock ctrlBlock) {
			super("CameraThread");
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "Constructor:");
		...
		public CameraServer getHandler() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "getHandler:");
			synchronized (mSync) {
				if (mHandler == null)
				try {
					mSync.wait();
		...
		public void handleOpen() {
			int opened;
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleOpen:");
			handleClose();
			synchronized (mSync) {
				mUVCCamera = new UVCCamera();
				opened = mUVCCamera.open(mCtrlBlock);
				if(opened != 0){
					Toast.makeText(mWeakContext.get(), "打开摄像头失败，请插拔一次摄像头", Toast.LENGTH_LONG).show();
					return;
				}
				if (UvcDebug.DEBUG) Log.i(TAG, "supportedSize:" + mUVCCamera.getSupportedSize());
				mUVCCamera.updateCameraParams();

			/*	Log.e(TAG, " 1 getHue();" + mUVCCamera.getHue() + " Ga " + mUVCCamera.getGamma());

				mUVCCamera.setGamma(valG);

				//new Handler().postDelayed(new Runnable() {public void run() {
					Log.e(TAG, " 2 getHue();" + mUVCCamera.getHue() + " Ga " + mUVCCamera.getGamma());*/
			//	}}, 1*1000);
			}
		...
		public void handleClose() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleClose:");
			handleStopRecording();
			...
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleClose:finished");
		}

		public void handleStartPreview(final int width, final int height, final Surface surface) {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleStartPreview:s");
			synchronized (mSync) {
...
//				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_YUV);
		...
		public void handleStopPreview() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleStopPreview:");
			synchronized (mSync) {
				if (mUVCCamera != null) {
					mUVCCamera.stopPreview();
				}
		...
		public void handleCaptureStill(final String path) {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleCaptureStill:");

			mSoundPool.play(mSoundId, 0.2f, 0.2f, 0, 0, 1.0f);	// play shutter sound
		}

		public void handleStartRecording() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleStartRecording:");
			try {
...
//				new MediaSurfaceEncoder(mFrameWidth, mFrameHeight, mMuxer, mMediaEncoderListener);
		...
		public void handleStopRecording() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleStopRecording:mMuxer=" + mMuxer);
			if (mMuxer != null) {
				synchronized (mSync) {
					if (mUVCCamera != null) {
		...
		public void handleUpdateMedia(final String path) {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleUpdateMedia:path=" + path);
			final Context context = mWeakContext.get();
			if (context != null) {
				try {
					if (UvcDebug.DEBUG) Log.i(TAG, "MediaScannerConnection#scanFile");
		...
		public void handleRelease() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "handleRelease:");
			handleClose();
			if (mCtrlBlock != null) {
				mCtrlBlock.close();
				mCtrlBlock = null;
...
/*		// if you need frame data as ByteBuffer on Java side, you can use this callback method with UVCCamera#setFrameCallback
		...
		private final IUVCServiceOnFrameAvailable mOnFrameAvailable = new IUVCServiceOnFrameAvailable() {
			...
			public IBinder asBinder() {
				if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "asBinder:");
				return null;
			}
			@Override
			public void onFrameAvailable() throws RemoteException {
//				if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "onFrameAvailable:");
		...
		private final MediaEncoder.MediaEncoderListener mMediaEncoderListener = new MediaEncoder.MediaEncoderListener() {
			...
			public void onPrepared(final MediaEncoder encoder) {
				if (UvcDebug.DEBUG) Log.d(TAG, "onPrepared:encoder=" + encoder);
				mIsRecording = true;
			...
			public void onStopped(final MediaEncoder encoder) {
				if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "onStopped:encoder=" + encoder);
				if ((encoder instanceof MediaSurfaceEncoder))
				try {
					mIsRecording = false;
		...
		private void loadShutterSound(final Context context) {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "loadShutterSound:");
		...
		@Override
		public void run() {
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "run:");
			Looper.prepare();
			synchronized (mSync) {
			...
			if (UvcDebug.DEBUG) Log.d(TAG_THREAD, "run:finished");
		}
	}
