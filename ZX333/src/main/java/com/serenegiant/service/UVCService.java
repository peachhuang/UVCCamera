...
import com.serenegiant.ZX333.MainActivity;
...
public class UVCService extends BaseService {
	...
	public void onCreate() {
		super.onCreate();
		if (UvcDebug.DEBUG) Log.d(TAG, "onCreate:");
		if (mUSBMonitor == null) {
...
/*	@Override
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		builder.setContentIntent(PendingIntent.getActivity(this, 0, nfIntent, 0)) // 设置PendingIntent
				.setLargeIcon(BitmapFactory.decodeResource(this.getResources(), R.drawable.recording)) // 设置下拉列表中的图标(大图标)
				.setContentTitle("ZXtriple") // 设置下拉列表里的标题
				.setSmallIcon(R.drawable.record) // 设置状态栏内的小图标
				.setContentText("usb camera is running") // 设置上下文内容
	...
	public void onDestroy() {
		if (UvcDebug.DEBUG) Log.d(TAG, "onDestroy:");
		if (mUSBMonitor != null) {
	...
	public IBinder onBind(final Intent intent) {
		if (UvcDebug.DEBUG) Log.d(TAG, "onBind:" + intent);
	...
	public void onRebind(final Intent intent) {
		if (UvcDebug.DEBUG) Log.d(TAG, "onRebind:" + intent);
	}

	@Override
	public boolean onUnbind(final Intent intent) {
		if (UvcDebug.DEBUG) Log.d(TAG, "onUnbind:" + intent);
		if (checkReleaseService()) {
			stopSelf();
		}
		if (UvcDebug.DEBUG) Log.d(TAG, "onUnbind:finished");
		return true;
	}
...
//********************************************************************************
	...
	private void showNotification(final CharSequence text) {
		if (UvcDebug.DEBUG) Log.v(TAG, "showNotification:" + text);
        // Set the info for the views that show in the notification panel.
        final Notification notification = new Notification.Builder(this)
			.setLargeIcon(BitmapFactory.decodeResource(this.getResources(), R.drawable.recording))
			.setSmallIcon(R.drawable.record)  // the status icon
			.setTicker(text)  // the status text
			.setWhen(System.currentTimeMillis())  // the time stamp
			.setContentTitle("ZXtriple")  // the label of the entry
			.setContentText("usb camera is running")  // the contents of the entry
	...
	private final OnDeviceConnectListener mOnDeviceConnectListener = new OnDeviceConnectListener() {
		...
		public void onAttach(final UsbDevice device) {
			if (UvcDebug.DEBUG) Log.d(TAG, "OnDeviceConnectListener#onAttach:");
		}

		@Override
		public void onConnect(final UsbDevice device, final UsbControlBlock ctrlBlock, final boolean createNew) {
			if (UvcDebug.DEBUG) Log.d(TAG, "OnDeviceConnectListener#onConnect:");
		...
		public void onDisconnect(final UsbDevice device, final UsbControlBlock ctrlBlock) {
			if (UvcDebug.DEBUG) Log.d(TAG, "OnDeviceConnectListener#onDisconnect:");
			queueEvent(new Runnable() {
				@Override
				public void run() {
		...
		public void onDettach(final UsbDevice device) {
			if (UvcDebug.DEBUG) Log.d(TAG, "OnDeviceConnectListener#onDettach:");
		}

		@Override
		public void onCancel(final UsbDevice device) {
			if (UvcDebug.DEBUG) Log.d(TAG, "OnDeviceConnectListener#onCancel:");
			synchronized (sServiceSync) {
				sServiceSync.notifyAll();
			}
		}
...
//********************************************************************************
	...
	private static boolean checkReleaseService() {
		...
		synchronized (sServiceSync) {
			...
			if (UvcDebug.DEBUG) Log.d(TAG, "checkReleaseService:number of service=" + n);
			for (int i = 0; i < n; i++) {
...
//********************************************************************************
	private final IUVCService.Stub mBasicBinder = new IUVCService.Stub() {
		...
		public int select(final UsbDevice device, final IUVCServiceCallback callback) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#select:device=" + (device !=null ? device.getDeviceName() : null));
			mCallback = callback;
		...
		public void release(final int serviceId) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#release:");
			synchronized (sServiceSync) {
		...
		public void releaseAll() throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#releaseAll:");
			CameraServer server;
			synchronized (sServiceSync) {
				final int n = sCameraServers.size();
				for (int i = 0; i < n; i++) {
		...
		public void resize(final int serviceId, final int width, final int height) {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#resize:");
		...
		public void connect(final int serviceId) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#connect:");
			final CameraServer server = getCameraServer(serviceId);
			if (server == null) {
				throw new IllegalArgumentException("invalid serviceId");
			}
		...
		public void disconnect(final int serviceId) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#disconnect:");
			final CameraServer server = getCameraServer(serviceId);
			if (server == null) {
				throw new IllegalArgumentException("invalid serviceId");
			}
		...
		public void addSurface(final int serviceId, final int id_surface, final Surface surface, final boolean isRecordable) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#addSurface:id=" + id_surface + ",surface=" + surface);
			final CameraServer server = getCameraServer(serviceId);
			if (server != null)
		...
		public void removeSurface(final int serviceId, final int id_surface) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#removeSurface:id=" + id_surface);
			final CameraServer server = getCameraServer(serviceId);
			if (server != null)
		...
		public void startRecording(final int serviceId) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#startRecording:");
			final CameraServer server = getCameraServer(serviceId);
		...
		public void stopRecording(final int serviceId) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#stopRecording:");
			final CameraServer server = getCameraServer(serviceId);
			if ((server != null) && server.isRecording()) {
		...
		public void captureStillImage(final int serviceId, final String path) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mBasicBinder#captureStillImage:" + path);
			final CameraServer server = getCameraServer(serviceId);
			if (server != null) {
...
//********************************************************************************
	private final IUVCSlaveService.Stub mSlaveBinder = new IUVCSlaveService.Stub() {
		...
		public void addSurface(final int serviceID, final int id_surface, final Surface surface, final boolean isRecordable, final IUVCServiceOnFrameAvailable callback) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mSlaveBinder#addSurface:id=" + id_surface + ",surface=" + surface);
			final CameraServer server = getCameraServer(serviceID);
			if (server != null) {
		...
		@Override
		public void removeSurface(final int serviceID, final int id_surface) throws RemoteException {
			if (UvcDebug.DEBUG) Log.d(TAG, "mSlaveBinder#removeSurface:id=" + id_surface);
			final CameraServer server = getCameraServer(serviceID);
			if (server != null) {
				server.removeSurface(id_surface);
			} else {
