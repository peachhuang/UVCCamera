...
public class MediaMuxerWrapper {
	...
	private static final String DIR_NAME = "ZXcam";
	...
	public MediaMuxerWrapper(String ext) throws IOException {
        ...
        //mOutputPath = "/storage/usbotg/My Novel.mp4";
...
//**********************************************************************
	...
	/*package*/ synchronized boolean start() {
		if (UvcDebug.DEBUG) Log.v(TAG,  "start:");
		mStatredCount++;
		if ((mEncoderCount > 0) && (mStatredCount == mEncoderCount)) {
			mMediaMuxer.start();
			mIsStarted = true;
			notifyAll();
			if (UvcDebug.DEBUG) Log.v(TAG,  "MediaMuxer started:");
		}
		return mIsStarted;
	}
	...
	*/
	/*package*/ synchronized void stop() {
		if (UvcDebug.DEBUG) Log.v(TAG,  "stop:mStatredCount=" + mStatredCount);
		mStatredCount--;
		if ((mEncoderCount > 0) && (mStatredCount <= 0)) {
			...
			mIsStarted = false;
			if (UvcDebug.DEBUG) Log.v(TAG,  "MediaMuxer stopped:");
		}
	}
	...
	/*package*/ synchronized int addTrack(final MediaFormat format) {
		...
		if (UvcDebug.DEBUG) Log.i(TAG, "addTrack:trackNum=" + mEncoderCount + ",trackIx=" + trackIx + ",format=" + format);
		return trackIx;
	}
