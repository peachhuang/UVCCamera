...
public class MediaAudioEncoder extends MediaEncoder implements IAudioEncoder {
	...
	public MediaAudioEncoder(final MediaMuxerWrapper muxer, final MediaEncoderListener listener) {
		super(muxer, listener, 1);
	}

	@Override
	protected void prepare() throws IOException {
		if (UvcDebug.DEBUG) Log.v(TAG, "prepare:");
        mTrackIndex = -1;
		...
		if (UvcDebug.DEBUG) Log.i(TAG, "selected codec: " + audioCodecInfo.getName());
...
//      audioFormat.setLong(MediaFormat.KEY_DURATION, (long)durationInMs );
		if (UvcDebug.DEBUG) Log.i(TAG, "format: " + audioFormat);
    ...
    private class AudioThread extends Thread {
    	...
    	public void run() {
			...
			if (audioRecord != null) {
				try {
					if (mIsCapturing) {
						if (UvcDebug.DEBUG) Log.v(TAG, "AudioThread:start audio recording");
						int readBytes;
			...
			if (UvcDebug.DEBUG) Log.v(TAG, "AudioThread:finished");
    	}
    }
    ...
    private static final MediaCodecInfo selectAudioCodec(final String mimeType) {
    	if (UvcDebug.DEBUG) Log.v(TAG, "selectAudioCodec:");
...
LOOP:	for (int i = 0; i < numCodecs; i++) {
            ...
            for (int j = 0; j < types.length; j++) {
            	if (UvcDebug.DEBUG) Log.i(TAG, "supportedType:" + codecInfo.getName() + ",MIME=" + types[j]);
