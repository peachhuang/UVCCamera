...
public class MediaVideoBufferEncoder extends MediaEncoder implements IVideoEncoder {
	...
	public MediaVideoBufferEncoder(final MediaMuxerWrapper muxer, final int width, final int height, final MediaEncoderListener listener) {
		super(muxer, listener, 3);
		if (UvcDebug.DEBUG) Log.i(TAG, "MediaVideoEncoder: ");
		mWidth = width;
		mHeight = height;
	}
...
//    	if (UvcDebug.DEBUG) Log.v(TAG, "encode:buffer=" + buffer);
	...
	protected void prepare() throws IOException {
		if (UvcDebug.DEBUG) Log.i(TAG, "prepare: ");
        mTrackIndex = -1;
		...
		if (UvcDebug.DEBUG) Log.i(TAG, "selected codec: " + videoCodecInfo.getName());
		...
		if (UvcDebug.DEBUG) Log.i(TAG, "format: " + format);

        mMediaCodec = MediaCodec.createEncoderByType(MIME_TYPE);
        mMediaCodec.configure(format, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
        mMediaCodec.start();
        if (UvcDebug.DEBUG) Log.i(TAG, "prepare finishing");
        if (mListener != null) {
        	try {
	...
	protected final MediaCodecInfo selectVideoCodec(final String mimeType) {
    	if (UvcDebug.DEBUG) Log.v(TAG, "selectVideoCodec:");
        ...
        for (int i = 0; i < numCodecs; i++) {
            ...
            for (int j = 0; j < types.length; j++) {
                if (types[j].equalsIgnoreCase(mimeType)) {
                	if (UvcDebug.DEBUG) Log.i(TAG, "codec:" + codecInfo.getName() + ",MIME=" + types[j]);
    ...
    protected static final int selectColorFormat(final MediaCodecInfo codecInfo, final String mimeType) {
		if (UvcDebug.DEBUG) Log.i(TAG, "selectColorFormat: ");
    	int result = 0;
...
//        	MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface,
    ...
    private static final boolean isRecognizedViewoFormat(final int colorFormat) {
		if (UvcDebug.DEBUG) Log.i(TAG, "isRecognizedViewoFormat:colorFormat=" + colorFormat);
