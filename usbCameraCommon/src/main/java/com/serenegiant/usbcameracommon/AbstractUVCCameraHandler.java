...
import com.serenegiant.encoder.MediaDecoder;
...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	public void close() {
		if (UvcDebug.DEBUG) Log.v(TAG, "close:");
		if (isOpened()) {
			stopPreview();
			sendEmptyMessage(MSG_CLOSE);
		}
		if (UvcDebug.DEBUG) Log.v(TAG, "close:finished");
	}
	...
	public void stopPreview() {
		if (UvcDebug.DEBUG) Log.v(TAG, "stopPreview:");
		...
		if (UvcDebug.DEBUG) Log.v(TAG, "stopPreview:finished");
	}
	...
	static final class CameraThread extends Thread {
		...
		private boolean bFilterSwitch = true;
		private boolean bFilterTipsSwitch = false;
		private static BlockingQueue<byte[]> video_Queue = new ArrayBlockingQueue<byte[]>(10000);
		/**
		 * shutter sound
		 */
		...
		public AbstractUVCCameraHandler getHandler() {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "getHandler:");
			synchronized (mSync) {
				if (mHandler == null)
				try {
					mSync.wait();
		...
		public void handleOpen(final USBMonitor.UsbControlBlock ctrlBlock) {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleOpen:");
			handleClose();
			try {
			...
			if (UvcDebug.DEBUG) Log.i(TAG, "supportedSize:" + (mUVCCamera != null ? mUVCCamera.getSupportedSize() : null));
		}

		public void handleClose() {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleClose:");
			handleStopRecording();
		...
		public void handleStartPreview(final Object surface) {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleStartPreview: " + mIsPreviewing);
			if ((mUVCCamera == null) || mIsPreviewing) {
				if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "return: ");
				return;
			}
			try {
			...
			///
		/*	mDecoder = new MediaDecoder((Surface)surface, 1);
			try {
				if (mWidth == 1280 && mHeight == 720) {
					mDecoder.initialHD();
				} else if (mWidth == 640 && mHeight == 360) {
					mDecoder.initialVGA();
				}
			}catch (Exception e) {
		...
		/*	mUVCCamera.setFrameCallback(new IFrameCallback() {
				...
				public void onFrame(ByteBuffer frame) {
					final byte[] data = new byte[frame.capacity()];
					frame.get(data);

					if (data[4] == 0x67) {
						Log.d("CarCam", "Get I frame" + " frame size = " + data.length);
						_pic = 2;
						_poc = 3;
						_drop = false;
					} else if (data[4] == 0x41) {
						Log.d("CarCam", "Get P frame(0x41)" + " frame size = " + data.length);
						_poc = data[7];
						if (_pic != data[6]) {
							_drop = true;
						}
						...
						Log.d("CarCam", "Get P frame(0x01)" + " frame size = " + data.length);
						_poc = data[7];
						if (_pic != data[6]) {
							_drop = true;
						}
						...
						_drop = true;
						Log.e("CarCam", "Get unknow frame" + String.format("(0x%x 0x%x 0x%x 0x%x 0x%x)", data[0], data[1], data[2], data[3], data[4]) + " frame size = " + data.length);
					}

					if (bFilterSwitch == false) {
						if(Const.DECODE_METHOD == Const.SW_DECODE) {
							try {
					...
					} else if (bFilterSwitch == true && _drop == false) {
						if(Const.DECODE_METHOD == Const.SW_DECODE) {
							try {
								video_Queue.put(data);
							} catch (InterruptedException e) {
					...
					if (bFilterTipsSwitch == true && _drop == true) {
						Log.d("CarCam", "drop frame" + String.format("(0x%x)", data[4]));
						runOnUiThread(new Runnable() {
							@Override
							public void run() {
								Log.d("CarCam", "run drop frame" + String.format("(0x%x)", data[4]));
								;//toast(get.this, "drop frame", 100);
							}
						});
					}
				}
		...
		public void handleStopPreview() {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleStopPreview:");
			if (mIsPreviewing) {
				if (mUVCCamera != null) {
			...
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleStopPreview:finished");
		}

		public void handleCaptureStill(final String path) {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleCaptureStill:");
		...
		public void handleStartRecording() {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleStartRecording:");
			try {
		...
		public void handleStopRecording() {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleStopRecording:mMuxer=" + mMuxer);
		...
		public void handleUpdateMedia(final String path) {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleUpdateMedia:path=" + path);
			final Activity parent = mWeakParent.get();
		...
		public void handleRelease() {
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleRelease:mIsRecording=" + mIsRecording);
			handleClose();
			mCallbacks.clear();
			if (!mIsRecording) {
			...
			if (UvcDebug.DEBUG) Log.v(TAG_THREAD, "handleRelease:finished");
		}

		private final MediaEncoder.MediaEncoderListener mMediaEncoderListener = new MediaEncoder.MediaEncoderListener() {
			@Override
			public void onPrepared(final MediaEncoder encoder) {
				if (UvcDebug.DEBUG) Log.v(TAG, "onPrepared:encoder=" + encoder);
				mIsRecording = true;
